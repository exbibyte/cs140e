...
impl Gpio<Uninitialized> {
    ...
    pub fn into_alt(self, function: Function) -> Gpio<Alt> {
        let fsel_idx = self.pin / 10;
        let offset = ( self.pin - fsel_idx * 10 ) * 3;
        (*self.registers).FSEL[ fsel_idx as usize ].or_mask( ( function as u32 ) << offset );
        self.transition()
    }
...
impl Gpio<Output> {
    ...
    pub fn set(&mut self) {
        if self.pin < 32 {
            // let val = (*(*self.registers).SET[0].ptr());
            (*self.registers).SET[0].write( 0b1 << self.pin );
        } else {
            // let val = (*(*self.registers).SET[1].ptr());
